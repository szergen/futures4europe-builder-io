...
const NEW_CSV_FILE =
  "./data/exports/Posts_Events_Project+Results+Pages_new.csv";
 ...
 */
async function migrate(count, isDryRun = false) {
  log.title("🚀 Starting Differential Post Pages Migration to Builder.io");

  // Load CSVs
  const oldPosts = loadCSV(OLD_CSV_FILE);
  const newPosts = loadCSV(NEW_CSV_FILE);

  // Identify new posts
  log.info("Comparing datasets to find new posts...");
  const oldIds = new Set(oldPosts.map((p) => p.ID));
  const newItems = newPosts.filter((p) => !oldIds.has(p.ID));

  if (newItems.length === 0) {
    log.success("No new posts found to migrate.");
    return;
  }

  log.success(`Found ${newItems.length} new posts to migrate.`);

  // Determine how many to migrate
  const postsToMigrate =
    count === "all" ? newItems : newItems.slice(0, parseInt(count));

  log.info(`Processing ${postsToMigrate.length} posts...`);

  if (isDryRun) {
    log.title("🧪 DRY RUN MODE - The following items would be migrated:");
    postsToMigrate.forEach((post, index) => {
      console.log(`  ${index + 1}. [${post.ID}] ${post.Title}`);
    });
    log.info(`\nTotal to migrate: ${postsToMigrate.length}`);
    return;
  }
  ...
  for (let i = 0; i < postsToMigrate.length; i++) {
    ...
    // Even though we filtered by ID, we check the mapping file too in case of partial runs
...
function showHelp() {
  console.log(`
${colors.bright}Wix Post Pages Differential Migration Script${colors.reset}

${colors.bright}Usage:${colors.reset}
  node scripts/migrations/migrate-posts-new.js <count> [--dry-run] [--validate]

${colors.bright}Arguments:${colors.reset}
  count       - Number of NEW posts to migrate (1, 10, 100, or "all")
  --dry-run   - Preview migration showing which items would be migrated
  --validate  - Validate migrated posts (not implemented)
...
${colors.bright}Description:${colors.reset}
  ...
  Identifies posts present in New but missing in Old (by ID).
  Migrates only these new posts.

${colors.bright}Examples:${colors.reset}
  node scripts/migrations/migrate-posts-new.js 10 --dry-run   # Show first 10 new posts
  node scripts/migrations/migrate-posts-new.js all            # Migrate all new posts
  `);
}
...
async function main() {
  ...
  try {
    if (isValidate) {
      ...
      // Standard migration mode (with optional dry-run)
      await migrate(count, isDryRun);
    }
  } catch (error) {
